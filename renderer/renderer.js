...
// Without this Electron opens dropped files as a new page, breaking drop zones.
document.addEventListener('dragover', (e) => e.preventDefault());
document.addEventListener('drop',     (e) => e.preventDefault());

// ══════════════════════════════════════════════════════════════════════════════
// 3D Preview — loaded dynamically so the UI still works if three.js is missing
// ══════════════════════════════════════════════════════════════════════════════

let preview = null; // set once Three.js loads
...
async function initPreview() {
  ...
  try {
    ...
    setPreviewStatus('3D preview unavailable — run: npm install');
    return;
  }

  // ── ModelPreview class (defined here so THREE is in scope) ─────────────────
  class ModelPreview {
    ...
    _initControls() {
      ...
      // Double-click: reset rotation and resume auto-spin
  ...
  preview = new ModelPreview('previewCanvas');
}

// Start loading Three.js — UI works regardless of outcome
initPreview();

// ══════════════════════════════════════════════════════════════════════════════
// UI — event listeners and form logic
// ══════════════════════════════════════════════════════════════════════════════
...
// ── Model file ────────────────────────────────────────────────────────────────
// <label for="modelFile"> already opens the dialog on click — no JS needed.
// Keyboard fallback (Enter / Space on focused label):
